...
)
{
	...
	case evmasm::Instruction::ADD:
		if (makesInfeasible(arguments.at(0) + arguments.at(1) >= smtutil::Expression(bigint(1) << 256)))
			m_solver->addAssertion(variable == arguments.at(0) + arguments.at(1));
		break;

	case evmasm::Instruction::SUB:
		if (makesInfeasible(arguments.at(0) - arguments.at(1) < 0))
			m_solver->addAssertion(variable == arguments.at(0) - arguments.at(1));
		break;
	...
	case evmasm::Instruction::LT:
		...
		m_nonEncodedValues.insert({_varName, arguments.at(0) < arguments.at(1)});
		break;
	...
	case evmasm::Instruction::ISZERO:
		m_solver->addAssertion(variable <= 1);
		m_nonEncodedValues.insert({_varName, !arguments.at(0)});
		break;
	case evmasm::Instruction::AND:
		m_solver->addAssertion(variable <= arguments.at(0) && variable <= arguments.at(1));
		m_nonEncodedValues.insert({_varName, arguments.at(0) && arguments.at(1)});
		break;
