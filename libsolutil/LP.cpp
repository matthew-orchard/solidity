...
pair<LPResult, Tableau> simplexEq(Tableau _tableau)
{
	...
	//cout << "LP: Too many iterations." << endl;
...
void LPSolver::addAssertion(Expression const& _expr)
{
	...
	else if (_expr.name == "<=")
	{
		...
		{
			//cout << "LPSolver: Adedd non-linear constraint." << endl;
			m_state.top().encounteredUnknownSituation = true;
		}
	}
	else if (_expr.name == ">=")
	...
	{
		m_state.top().encounteredUnknownSituation = true;
		//cout << "LPSolver: Unknown operation: " + _expr.name << endl;
	}
}
